...
- **[Spec 2i.1] Shared inputs** — `shared_inputs` + `shared_input_fields`: a bulk input applied to a chosen set of fields, split `per_ha_rate` (rate × ha) or `total_split_ha` (by ha-share). `services/shared_inputs.js` → `fieldSharedInputCost`; `computeFieldCop` opt-in `include=shared` attaches `line.shared_input_cost` (enterprise line → single productive line → `shared_input_line_not_found`), excludes `is_establishment` (→ capital), flags `coverage.excluded_layers` when off. Provenance columns (`entry_basis`/`external_source`/`external_id`, idempotent unique index) ready Xero/QuickBooks import. CRUD API. Spec: `docs/specs/spec-2i-activity-based-field-costing.md`. Next: 2i.2 equipment rates → 2i.3 activities → 2i.4 establishment amortise; then 2h node map; future 2j Xero

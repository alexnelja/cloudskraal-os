...
- **Spec 2e** — Rooibos processing centre: **2e.1 shipped** (`processing_batches` + `processing_batch_sources`, `services/processing.js` batchYield + fieldProcessingShare; `computeFieldCop` opt-in `include=processing` attaches batch-actual processing cost + `cost_per_netto_kg_actual` from real shrinkage; CRUD API). **2e.2 queued**: stokke recirculation (non-double-counting) + byproduct revenue

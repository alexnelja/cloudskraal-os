...
- **[Spec 2h.2] Cost Map page** — `/cost-map` (sidebar › Business): left→right SVG cost build-up DAG (`components/costmap/CostNodeMapView`, deterministic layered layout, per-layer colours, off/no-data layers dashed with enable hints, click-to-toggle = `include` flags refetch). KPI strip (total, cost/kg, price, margin, Cloudskraal avg), denominator pills (harvest/dried/netto via 2b tiers), scope toggle Field ↔ Cloudskraal (farm view: per-field R/kg table vs weighted avg, click-through to field). **What-if editing**: click any leaf/layer/yield/price node → override → client-side recompute (`lib/costMapWhatIf.applyWhatIf`, amber deltas, reset); backend summary gained `yield_at_price_basis_kg` so margin re-derives under yield overrides. Remaining 2h ideas: enterprise comparison page, data-quality widget (benched spec). Next: 2j Xero / COP UI polish
